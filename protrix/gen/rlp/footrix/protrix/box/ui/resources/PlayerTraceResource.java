...
import rlp.footrix.protrix.box.ui.pages.PlayerTracePage;
...
public class PlayerTraceResource extends io.intino.alexandria.ui.spark.resources.Resource {
	private final ProtrixBox box;

	public PlayerTraceResource(ProtrixBox box, io.intino.alexandria.ui.spark.UISparkManager manager, DisplayNotifierProvider notifierProvider) {
	...
	private void render() {
		...
		UiPage page = new PlayerTracePage();
		...
		page.session.whenLogin(new Function<String, String>() {
			...
			public String apply(String baseUrl) {
				return PlayerTraceResource.this.authenticate(page.session, baseUrl);
			}
		});
		page.session.whenLogout(b -> logout(page.session));
        ((PlayerTracePage)page).box = box;
		page.clientId = clientId;
		page.device = parameterValue("device");
		page.token = parameterValue("token");
		((PlayerTracePage)page).playerId = parameterValue("playerId");
		...
		manager.pushService().onOpen(client -> {
			...
			if (client.soul() != null) {
				((rlp.footrix.protrix.box.ui.displays.RouteDispatcher)box.routeManager().routeDispatcher()).dispatchPlayerTrace(client.soul(), ((PlayerTracePage)page).playerId);
				return false;
			}
			...
			((rlp.footrix.protrix.box.ui.displays.RouteDispatcher)box.routeManager().routeDispatcher()).dispatchPlayerTrace(soul, ((PlayerTracePage)page).playerId);
