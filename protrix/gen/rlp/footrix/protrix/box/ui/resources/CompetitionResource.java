...
import rlp.footrix.protrix.box.ui.pages.CompetitionPage;
...
public class CompetitionResource extends io.intino.alexandria.ui.spark.resources.Resource {
	private final ProtrixBox box;

	public CompetitionResource(ProtrixBox box, io.intino.alexandria.ui.spark.UISparkManager manager, DisplayNotifierProvider notifierProvider) {
	...
	private void render() {
		...
		UiPage page = new CompetitionPage();
		...
		page.session.whenLogin(new Function<String, String>() {
			...
			public String apply(String baseUrl) {
				return CompetitionResource.this.authenticate(page.session, baseUrl);
			}
		});
		page.session.whenLogout(b -> logout(page.session));
        ((CompetitionPage)page).box = box;
		page.clientId = clientId;
		page.device = parameterValue("device");
		page.token = parameterValue("token");
		((CompetitionPage)page).competitionId = parameterValue("competitionId");
		((CompetitionPage)page).season = parameterValue("season");
		...
		manager.pushService().onOpen(client -> {
			...
			if (client.soul() != null) {
				((rlp.footrix.protrix.box.ui.displays.RouteDispatcher)box.routeManager().routeDispatcher()).dispatchCompetition(client.soul(), ((CompetitionPage)page).competitionId, ((CompetitionPage)page).season);
				return false;
			}
			...
			((rlp.footrix.protrix.box.ui.displays.RouteDispatcher)box.routeManager().routeDispatcher()).dispatchCompetition(soul, ((CompetitionPage)page).competitionId, ((CompetitionPage)page).season);
