...
import rlp.footrix.protrix.box.ui.pages.TeamsPage;
...
public class TeamsResource extends io.intino.alexandria.ui.spark.resources.Resource {
	private final ProtrixBox box;

	public TeamsResource(ProtrixBox box, io.intino.alexandria.ui.spark.UISparkManager manager, DisplayNotifierProvider notifierProvider) {
	...
	private void render() {
		...
		page.session.whenLogin(new Function<String, String>() {
			...
			public String apply(String baseUrl) {
				return TeamsResource.this.authenticate(page.session, baseUrl);
			}
		});
		page.session.whenLogout(b -> logout(page.session));
        ((TeamsPage)page).box = box;
		...
		manager.pushService().onOpen(client -> {
			...
			if (client.soul() != null) {
				((rlp.footrix.protrix.box.ui.displays.RouteDispatcher)box.routeManager().routeDispatcher()).dispatchTeams(client.soul());
				return false;
			}
			...
			((rlp.footrix.protrix.box.ui.displays.RouteDispatcher)box.routeManager().routeDispatcher()).dispatchTeams(soul);
