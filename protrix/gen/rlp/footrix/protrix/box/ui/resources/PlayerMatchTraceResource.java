...
import rlp.footrix.protrix.box.ui.pages.PlayerMatchTracePage;
...
public class PlayerMatchTraceResource extends io.intino.alexandria.ui.spark.resources.Resource {
	private final ProtrixBox box;

	public PlayerMatchTraceResource(ProtrixBox box, io.intino.alexandria.ui.spark.UISparkManager manager, DisplayNotifierProvider notifierProvider) {
	...
	private void render() {
		...
		UiPage page = new PlayerMatchTracePage();
		...
		page.session.whenLogin(new Function<String, String>() {
			...
			public String apply(String baseUrl) {
				return PlayerMatchTraceResource.this.authenticate(page.session, baseUrl);
			}
		});
		page.session.whenLogout(b -> logout(page.session));
        ((PlayerMatchTracePage)page).box = box;
		page.clientId = clientId;
		page.device = parameterValue("device");
		page.token = parameterValue("token");
		((PlayerMatchTracePage)page).playerId = parameterValue("playerId");
		...
		manager.pushService().onOpen(client -> {
			...
			if (client.soul() != null) {
				((rlp.footrix.protrix.box.ui.displays.RouteDispatcher)box.routeManager().routeDispatcher()).dispatchPlayerMatchTrace(client.soul(), ((PlayerMatchTracePage)page).playerId);
				return false;
			}
			...
			((rlp.footrix.protrix.box.ui.displays.RouteDispatcher)box.routeManager().routeDispatcher()).dispatchPlayerMatchTrace(soul, ((PlayerMatchTracePage)page).playerId);
