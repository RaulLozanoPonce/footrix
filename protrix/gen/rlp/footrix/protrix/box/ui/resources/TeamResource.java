...
import rlp.footrix.protrix.box.ui.pages.TeamPage;
...
public class TeamResource extends io.intino.alexandria.ui.spark.resources.Resource {
	private final ProtrixBox box;

	public TeamResource(ProtrixBox box, io.intino.alexandria.ui.spark.UISparkManager manager, DisplayNotifierProvider notifierProvider) {
	...
	private void render() {
		...
		manager.pushService().onOpen(client -> {
			...
			if (client.soul() != null) {
				((rlp.footrix.protrix.box.ui.displays.RouteDispatcher)box.routeManager().routeDispatcher()).dispatchTeam(client.soul(), ((TeamPage)page).teamId);
				return false;
			}
			...
			((rlp.footrix.protrix.box.ui.displays.RouteDispatcher)box.routeManager().routeDispatcher()).dispatchTeam(soul, ((TeamPage)page).teamId);
