...
public class AppTemplate extends AbstractAppTemplate<ProtrixBox> {
	...
	private void renderTableItem(AddCollectionItemEvent event) {
        ...
        PlayerMatchRecord record = team == null ? null : box().application().recordStore().playerMatchRecord(player.definition().id(), team.definition().id(), "ESP-1", 0);
		...
		row.minutesMold.minutes.value(record == null ? null : record.playedMinutes() / (double) record.maxMinutes());
		row.cacheMold.cache.value(player.cache().relativeCache());
	}
}